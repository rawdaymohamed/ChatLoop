...
- **Delete for everyone** — soft-delete sets `softDeleted: true`; message shows as _"This message was deleted"_ tombstone for all members
...
- **Multi-device / multi-tab aware** — `Map<userId, Set<socketId>>` tracks all open sockets; user is only marked offline when their _last_ socket closes
...
- **Typing indicators** — `typing` / `stop-typing` events broadcast to the conversation room _and_ to the receiver's personal room if they are online but not viewing that chat

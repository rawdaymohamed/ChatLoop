...
function sanitizeForRequester(member, requesterId) {
  ...
  return {
    ...
    about: "",
    profilePic:
      "https://ui-avatars.com/api/?name=Conversa+User&background=6366f1&color=fff&bold=true",
    isOnline: false,
    lastSeen: null,
    isBot: obj.isBot,
    createdAt: null,
...
const togglePin = async (req, res) => {
  ...
  try {
    ...
    const isPinned = user.pinnedConversations.some(
      (id) => id.toString() === convId,
    );

    if (isPinned) {
      await User.findByIdAndUpdate(userId, {
        $pull: { pinnedConversations: convId },
      });
      return res.status(200).json({ isPinned: false });
    } else {
      await User.findByIdAndUpdate(userId, {
        $addToSet: { pinnedConversations: convId },
      });

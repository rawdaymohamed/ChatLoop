...
const register = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error(
        "Cleanup after failed registration also failed:",
...
const sendotp = async (req, res) => {
  try {
    ...
    let mailDetails = {
      from: `"ChatLoop" <${EMAIL}>`,
      to: email,
      subject: "Your ChatLoop Login OTP - " + otp,
      html: `<!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Your ChatLoop OTP</title>
  </head>
    ...
    <tr>
      ...
      <table width="520" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <td style="padding:40px 40px 32px;">
          ...
          We received a request to sign in to your ChatLoop account. Use the one-time password below to complete your login.
          </p>
...
const sendVerificationOtp = async (req, res) => {
  try {
    ...
    const mailDetails = {
      from: `"ChatLoop" <${EMAIL}>`,
      to: user.email,
      subject: `Verify your ChatLoop email – OTP: ${otp}`,
      html: `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
